...
export async function migrateGenerate(): Promise<void> {
  ...
  const schemaExists = await fs.access(schemaDir).then(() => true).catch(() => false);
  if (!schemaExists) {
    console.log("❌ No schema folder found. Please run 'grwo generate <openapi.yml>' first to generate schemas.");
    process.exit(0);
  }
  ...
  if (tsFiles.length === 0) {
    console.log("❌ No schema files found in src/db/schema/. Please run 'grwo generate <openapi.yml>' first.");
    process.exit(0);
  }
  ...
  if (!await fs.access(migrationsDir).then(() => true).catch(() => false)) {
